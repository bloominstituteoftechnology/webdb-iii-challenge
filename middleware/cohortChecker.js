    ...
    cohortDB("cohorts")
      .then(cohorts => {
        ...
        if (exists === true){
          ...
          res.stats(404).json({Error: `${req.body.cohort_id} is not an existing cohort id`})
        }
      })
      .catch(error => {
        res
          .status(500)
          .json({
            Error: "There was a problem obtaining the list of cohort ids to compare with the cohort_id on the student you were adding",
            error
          });
      });
    next()
