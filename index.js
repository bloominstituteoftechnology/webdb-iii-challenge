...
server.get('/api/cohorts/:id', (req, res) => {
    ...
    db('cohorts')
        ...
        .catch(() => {
            res
                .status(500)
                .json({ error: 'Failed to find a cohort with this id in the database.'})
        })
})
...
server.delete('/api/cohorts/:id', (req, res) => {
    const { id } = req.params
    db('cohorts').where({ id }).del() //also can use (id : id) or ('id', id)
    .then(count => {
        if (count) {
...
server.post('/api/students', (req, res) => {
  ...
  server.get('/api/students/:id', (req, res) => {
      ...
      db('students')
          ...
          .catch(() => {
              res
                  .status(500)
                  .json({ error: 'Failed to find a student with this id in the database.'})
          })
  })
  ...
  server.delete('/api/students/:id', (req, res) => {
      const { id } = req.params
      db('students').where({ id }).del() //also can use (id : id) or ('id', id)
      .then(count => {
          if (count) {
              res.json({ message: 'The student was successfully deleted from the database.'})
          } else {
              res.status(404).json({ error: 'The student with the specified id does not exist in the database.'})
          }
      })
      .catch(err => {
          res.status(500).json({ error: 'The student could not be removed from the database.'})
      })
  })
